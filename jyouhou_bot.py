...
def send_flex_message(items):
    ...
    # 1. 商品グループの処理（看板画像＋商品のカルーセル）
    if product_items:
        # ★商品の前だけに独立した看板画像を配置（これが一番美しく、文字も切れません）
        messages_payload.append({
            "type": "image",
    ...
    # ★安全装置: payload配列が5を超える場合は分割（LINEの仕様上、1リクエストにつき5枠まで）
    # ※ブログと商品が同時に来ても、画像1＋商品1＋ブログ1＝3枠なので、1リクエスト（1通分）でまとめて送られます。
