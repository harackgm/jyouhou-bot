...
MAX_NOTIFY_LIMIT = (
    10  # 大量検知時の事故防止ガード（10件超は通知スキップして自動既読化）
)
...
def is_notified(item_key):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT 1 FROM notified_products WHERE item_key = ?", (item_key,)
    )
...
def save_notified_bulk(items):
    ...
    cursor = conn.cursor()
    cursor.executemany(
        "INSERT OR IGNORE INTO notified_products (item_key, title, url)"
        " VALUES (?, ?, ?)",
...
def extract_image_from_detail_page(detail_url, headers):
    ...
    try:
        ...
        img_tag = soup.select_one(
            ".product_image img, .img_box img, #product_image img, .product-img"
            " img"
        )
...
def fetch_product_image_url(target_url, headers):
    ...
    try:
        ...
        first_product_a = soup.select_one(
            ".product_list a, .item_list a, .product_data a, table.product a,"
            " .info_detail a, a[href*='pid=']"
        )
        if first_product_a and first_product_a.get("href"):
            deep_url = requests.compat.urljoin(
                target_url, first_product_a["href"]
            )
...
def send_flex_message(items):
    ...
    for i in range(0, len(items), chunk_size):
        ...
        for title, product_url, img_url in chunk:
            display_title = (
                title.strip() if (title and title.strip()) else "新着・再入荷情報"
            )
            ...
            "messages": [
                {
                    "type": "flex",
                    "altText": (
                        f"城峰釣具店 INFORMATION新着通知 ({len(chunk)}件)"
                    ),
...
def get_top_information_items(soup):
    """div.info エリアから「ラベル＋商品名」の全テキストとリンクを確実に取得"""
    ...
    # div.info 内の各行要素（li, p, divなど）または直接のaタグから抽出
    # 親要素全体からテキストを取得することで「再入荷」「NEW」などのラベル落ちを防ぐ
    elements = info_div.find_all(["li", "p", "dt", "dd", "tr"])
    if not elements:
        # 特別なブロック要素がない場合はaタグの親要素を基準にする
        elements = [
            a.parent
            for a in info_div.find_all("a", href=True)
            if a.parent != info_div
        ]

    if not elements:
        # 万が一親要素でうまく取れない場合のフォールバック
...
def main():
    ...
    if get_db_count() == 0:
        print(
            f"[INFO] 初回データベース初期化: 現存する{len(all_items)}件を既読登録します。"
        )
    ...
    if len(new_items) > MAX_NOTIFY_LIMIT:
        print(
            f"[WARN] 新規検知が{len(new_items)}件と多いため、LINE通知枠保護により送信をスキップしてDBを更新します。"
        )
